...
def main():
	...
	st.set_page_config(page_title="Wyrocznia.exe")
	...
	with left:
		plec_radio = st.radio( "Płeć", list(sex_d.keys()), format_func=lambda x : sex_d[x] )
		embarked_radio = st.radio( "Port zaokrętowania", list(embarked_d.keys()), index=2, format_func= lambda x: embarked_d[x] )
		pclass_radio = st.radio( "Klasa", list(pclass_d.keys()), format_func=lambda x : pclass_d[x] )

	with right:
		wiek_slider = st.slider("Wiek", value=1, min_value=1, max_value=80)
